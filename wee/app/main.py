...
@app.get("/image/scan/ya")
async def scan_image_ya(url: str):
  image = Image.open(requests.get(url, stream=True).raw)  
  image.save("label.png")
  text = pytesseract.image_to_string(Image.open("label.png"), lang="tha+eng")
  prompts = """
ต่อไปนี้เป็นข้อมูลจากฉลากยาที่ผู้ใช้ได้รับ:

กรุณาช่วยวิเคราะห์ข้อมูล และสรุปว่า:
1. ผู้ใช้ควรกินยานี้วันละกี่ครั้ง
2. ควรกินช่วงเวลาใดบ้าง เช่น ก่อนอาหาร เช้า กลางวัน เย็น หรือก่อนนอน
3. ในแต่ละช่วง ควรกินกี่เม็ด เช่น เช้า 2 เม็ด เย็น 1 เม็ด เป็นต้น
   - หากไม่ได้กินในบางช่วงเวลา ให้ใส่ "-" แทนจำนวนนั้น เช่น "กลางวัน": "-"
4. มีข้อควรระวังอะไรหรือไม่ เช่น ห้ามกินตอนท้องว่าง ห้ามดื่มแอลกอฮอล์ ฯลฯ

กรุณาตอบกลับมาเป็น JSON object โดยใช้รูปแบบดังนี้เช่นนี้:
{
  "doses_per_day": 2,
  "dosage_schedule": {
    "morning": 2,
...
ตอบข้อมูลมาตามที่มีเท่านั้น และอย่าตอบอะไรเพิ่มเติมนอกจาก JSON object นี้
"""
  finall_answer = await call_naja(prompts,temperature=0.3,user_input=text)
  finall_answer = finall_answer.split("```json")[1].split("```")[0]
  finall_answer = json.loads(finall_answer)
  return finall_answer
...
@app.get("/yang")
async def yak_yang(sec :int,url : int):
  if sec == 1:
    return PlainTextResponse(f"""📏 ขนาดยาง 195/65R15 91V
🛞 หน้ายาง: 195 มม.
📐 แก้มยางสูง: 65% ของหน้ายาง
⚙️ ขนาดล้อ: 15 นิ้ว
💪 รับน้ำหนักสูงสุด: 615 กก./เส้น
🚀 ความเร็วสูงสุด: 240 กม./ชม.
              """)
  if sec == 2:
    return PlainTextResponse(f"""
🔧 ยางมีความเสียหายบางส่วน
📉 ระดับความเสียหาย: ปานกลาง - อาจยังใช้งานได้ แต่ประสิทธิภาพลดลง
⏳ แนะนำให้เปลี่ยนภายใน 1-3 เดือน เพื่อความปลอดภัยสูงสุดครับ/ค่ะ""")
  else:
...
async def get_location(location: str):
    try:
      ...
      return PlainTextResponse("❌ รูปแบบ location ต้องเป็น lat,lon,info", status_code=400)
    radius = 30000 
    ...
    if response.status_code != 200:
        return PlainTextResponse("❌ ไม่สามารถเชื่อมต่อ Overpass API", status_code=500)

    elements = response.json().get("elements", [])

    if not elements:
        return PlainTextResponse("❗ ไม่พบร้านยางในรัศมี", status_code=404)
    ...
    name = closest.get("tags", {}).get("name", "ไม่มีชื่อร้าน")
    nearest_lat = closest["lat"]
    nearest_lon = closest["lon"]
    distance = int(haversine(lat, lon, nearest_lat, nearest_lon))

    result = f"""🏁 ร้านยางที่ใกล้ที่สุด:
📍 พิกัด: {nearest_lat},{nearest_lon}
🏪 ชื่อ: {name}
🛣️ ระยะทาง: {distance} เมตร
🌐 แผนที่: https://maps.google.com/?q={nearest_lat},{nearest_lon}
